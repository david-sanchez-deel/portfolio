...
env = gym.make('FrozenLake-v0', is_slippery=False) # Or true
...
def q_learning(episodes, epsilon=0.1, step_size=0.01, gamma=0.9):
    ...
    policy_data = np.array([np.array([1/posible_actions for i in range(posible_actions)]) for i in range(posible_states)])
...
policy_data, q = q_learning(500)

json.dump(policy_data.tolist(), codecs.open("./policy_data.json", 'w', encoding='utf-8'), separators=(',', ':'), sort_keys=True, indent=4)
json.dump(q.tolist(), codecs.open("./q.json", 'w', encoding='utf-8'), separators=(',', ':'), sort_keys=True, indent=4)
